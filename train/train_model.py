"""
GlucoseGuard Model Training Script
Trains Random Forest, Logistic Regression, and SVM on Pima Indians Diabetes Dataset
Saves models + scaler + metrics to models/ directory
"""
...
# Source: UCI ML Repository — 768 instances, 8 features
PIMA_DATA = """pregnancies,glucose,bp,skin,insulin,bmi,dpf,age,outcome
...
5,139,80,35,270,31.6,0.361,25,1
...
0,120,74,0,0,18.4,0.449,25,0
1,82,64,13,95,21.2,0.415,23,0
2,214,110,0,0,31.9,0.966,30,1
5,130,82,0,0,39.1,0.956,37,1
"""
...
def load_pima_data():
    ...
    df = pd.read_csv(StringIO(PIMA_DATA.strip()))

    # Zero-impute medically invalid zeros
    zero_invalid = ["glucose", "bp", "skin", "insulin", "bmi"]
...
def train():
    ...
    # ── Load data ──────────────────────────────────────────────────────────
    df = load_pima_data()
    print(f"Dataset: {len(df)} rows | {df['outcome'].sum()} diabetic ({df['outcome'].mean()*100:.1f}%)")

    feature_cols = ["pregnancies", "glucose", "bp", "skin", "insulin", "bmi", "dpf", "age"]
    ...
    joblib.dump(rf, os.path.join(MODELS_DIR, "diabetes_rf.pkl"))
    print(f"  → Saved diabetes_rf.pkl")
    ...
    lr_metrics = evaluate_model(lr, X_test_sc, y_test, "Logistic Regression")
    all_metrics.append(lr_metrics)
    joblib.dump(lr, os.path.join(MODELS_DIR, "diabetes_lr.pkl"))
    print(f"  → Saved diabetes_lr.pkl")
    ...
    svm_metrics = evaluate_model(svm, X_test_sc, y_test, "SVM")
    all_metrics.append(svm_metrics)
    joblib.dump(svm, os.path.join(MODELS_DIR, "diabetes_svm.pkl"))
    print(f"  → Saved diabetes_svm.pkl")

    # ── Save scaler + feature list + metrics ───────────────────────────────
    ...
    print("   Files: diabetes_rf.pkl, diabetes_lr.pkl, diabetes_svm.pkl,")
    print("          scaler.pkl, feature_cols.pkl, metrics.json, feature_importance.json")
